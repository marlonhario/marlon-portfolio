import {
  FaHtml5,
  FaCss3,
  FaJs,
  FaReact,
  FaFigma,
  FaNodeJs,
} from "react-icons/fa";
import {
  SiTailwindcss,
  SiNextdotjs,
  SiSass,
  SiStyledcomponents,
  SiShadcnui,
  SiRedux,
  SiReactquery,
  SiEslint,
  SiPrettier,
  SiVite,
  SiStripe,
  SiAuth0,
  SiJsonwebtokens,
  SiVercel,
  SiJest,
  SiTestinglibrary,
  SiPhp,
  SiMysql,
  SiPostgresql,
  SiMongodb,
  SiSqlite,
  SiBootstrap,
  SiLaravel,
  SiExpress,
  SiGraphql,
  SiApollographql,
  SiSocketdotio,
  SiAxios,
  SiPostman,
  SiTypescript,
  SiXampp,
  SiSublimetext,
  SiNotepadplusplus,
  SiGit,
  SiSupabase,
} from "react-icons/si";

export const experience = {
  icon: "/assets/resume/badge.svg",
  title: "My Experience",
  description:
    "Senior React and Full Stack Developer with 6+ years of experience building enterprise SaaS platforms, healthcare systems, fintech applications, learning management systems, real-time communication platforms, and business management solutions. Proven expertise in React, Next.js, TypeScript, Node.js, GraphQL, AWS, PostgreSQL, and modern frontend architecture. Experienced in leading frontend initiatives, implementing scalable component libraries, improving code quality, and delivering high-performance applications across distributed teams.",
  items: [
    {
      company: "IT-SPAC",
      position: "Senior React Developer",
      duration: "Oct 2024 – Aug 2025",
      project: "EncompaaS (Information Management Platform)",
      url: "https://encompaas.cloud/",
      techs: "Typescript, React JS, Next JS, MUI, OData and Storybook.",
      responsibilities: [
        "Built scalable, reusable component systems to improve development consistency.",
        "Delivered features from Figma with strong UI/UX accuracy.",
        "Refactored legacy codebases for maintainability and performance.",
        "Implemented end-to-end testing with Playwright.",
        "Led frontend development in a microservices architecture.",
        "Conducted code reviews to raise team code quality.",
      ],
    },
    {
      company: "XAM",
      position: "Senior React Developer",
      duration: "Apr 2024 – Oct 2024",
      project: "thegrowthfaculty (Online Event and Learning Management System)",
      url: "https://thegrowthfaculty.com/",
      techs: "Typescript, React JS, Next JS and Maestro.io.",
      responsibilities: [
        "Built live event features(Maestro.io) on the existing platform.",
        "Developed scalable backend APIs.",
        "Implemented monitoring and error tracking (Sentry, New Relic).",
        "Enforced code quality via reviews and best practices.",
        "Standardized tooling with Husky, ESLint, and Prettier.",
      ],
    },
    {
      company: "XAM",
      position: "Senior React Developer",
      duration: "Mar 2024 – Apr 2024",
      project: "DrTalk (HIPAA-compliant real-time platform)",
      url: "https://www.drtalk.com/",
      techs: "Typescript, React JS, Next JS and SignalR.",
      responsibilities: [
        "Focused on front-end development.",
        "Built real-time features using SignalR (DrTalk).",
        "Created and shared coding best practices across teams.",
        "Developed reusable components for multiple projects.",
        "Resolved chat box bugs.",
      ],
    },
    {
      company: "Adaca",
      position: "Front-end Developer",
      duration: "Jun 2022 – May 2023",
      project: "PRISM (Market Investment App)",
      url: "https://www.prism.markets/",
      techs:
        "Typescript, React JS, Recharts, Redux, Axios, React Query and sass.",
      responsibilities: [
        "Built data visualization charts and market graphs.",
        "Implemented features from Adobe XD mockups.",
        "Managed state with Redux.",
        "Fixed bugs and improved error handling.",
        "Applied best practices in Sass, React hooks, and state management.",
      ],
    },
    {
      company: "Adaca.",
      position: "Front-end Developer",
      duration: "Oct 2021 – Jun 2022",
      project: "gsquared. (Task Management System)",
      url: "https://www.gsquared.com.au/",
      techs:
        "Typescript, React JS, React Query, Axios, Material UI, React native, redux tookit, Redux and sass.",
      responsibilities: [
        "Built UI from scratch using TypeScript, React, MUI, Axios, and React Query.",
        "Created reusable components, hooks, and utilities following SOLID principles.",
        "Implemented state management and data caching.",
        "Developed features from Adobe XD mockups.",
      ],
    },
    // {
    //   company: "Event Espresso",
    //   position: "Front-end Developer",
    //   duration: "Oct 2021 – Jun 2022",
    //   project: "Wordpress Ticketing Plugin",
    //   url: "https://eventespresso.com/",
    //   techs:
    //     "Typescript, React JS, React Query, Axios, SASS, Jest, Playwright and Redux.",
    //   responsibilities: [
    //     "Set up Jest for front-end testing.",
    //     "Created unit tests for front-end components and functions.",
    //     "Developed new features for plugins.",
    //     "Assisted in fixing bugs and resolving issues.",
    //   ],
    // },
    {
      company: "Prospertogether",
      position: "Full-stack Developer",
      duration: "Jul 2020 - Oct 2021",
      project: "Trading Platform",
      url: "http://www.prospertogether.com/",
      techs:
        "Typescript, React JS, D3-shape, React Native, MongoDB, NoSQL, Graphql, Apollo, express, node js, Redux, React Query, SASS, React native, redux tookit, JWT, Nodemailer, Jest, Cypress, PostgreSQL, SQL, Blockly, Chart js, SocketLabs, Plaid and Alpaca.",
      responsibilities: [
        "Configured PostgreSQL and built optimized queries.",
        "Set up Apollo Client/Server for frontend–backend communication.",
        "Implemented JWT auth, middleware, and user access control.",
        "Applied MVC architecture and best practices.",
        "Integrated Plaid and Alpaca APIs for banking and crypto trading.",
        "Built charts, crypto data views, and user management features.",
        "Managed frontend state with Redux and implemented mailing functionality.",
      ],
    },
    {
      company: "Sellgo",
      position: "Front-end Developer",
      duration: "Jan 2020 - Jul 2020",
      project: "Amazon Seller Database",
      url: "https://www.linkedin.com/company/sellgo/",
      techs:
        "Typescript, React JS, Axios, SASS, csv-parse, highcharts, lodash, react-helmet, Semantic UI and Redux.",
      responsibilities: [
        "Built UI with React, TypeScript, Redux, and modern libraries.",
        "Developed multi-step forms, auth (login/register), and responsive layouts.",
        "Implemented CSV uploads, data tables with filters/search, and API integrations.",
        "Created user guides, video modules, and onboarding features.",
        "Assisted in refactoring, bug fixing, and state management.",
      ],
    },
    {
      company: "Segworks",
      position: "Full-stack Developer",
      duration: "May 2019 - Dec 2019",
      project: "Hospital management and healthcare services System",
      url: "https://www.segworks.com/",
      techs:
        "Typescript, Javascript, MySQL, MongoDB, NoSQL, Express, Node JS, React JS, html/html5, css/css3, Ajax, jQuery, Bootstrap, PHP, Yii PHP, Jasper Reports, Laravel, Vue JS and Redux.",
      responsibilities: [
        "Developed full-stack features using React, Node.js, PHP, and SQL/NoSQL databases.",
        "Built report generation for billing and clinical data, including print formats (JasperReports).",
        "Created clinical and billing form UIs with complete functionality.",
        "Implemented backend APIs and database queries to support system features.",
      ],
    },
    {
      company: "Freelance",
      position: "Front-end Developer",
      duration: "May 2019 - Dec 2019",
      project: "Blogs, Magazines and Hybrid landing page",
      url: "https://www.segworks.com/",
      techs: "Wordpress, Divi theme, HTML5, SASS.",
      responsibilities: [
        "Designed landing page mockups using Adobe Photoshop.",
        "Created banners, logos, and graphic assets with Adobe Illustrator and Photoshop.",
        "Built responsive WordPress websites using the Divi theme.",
        "Installed and configured WordPress plugins and themes.",
        "Customized websites based on client requirements.",
      ],
    },
    {
      company: "WISE",
      position: "Full-stack Developer",
      duration: "Jun 2017 - May 2018",
      project: "Real Estate Application & Eatery POS with Inventory Management",
      url: "",
      techs:
        "Typescript, Javascript, MySQL, NoSQL, MongoDB, React JS, Node JS, Express, html/html5, css/css3, Ajax, jQuery, Bootstrap, PHP, Laravel, Vue JS, Vagrant, Homestead, Redux and SASS.",
      responsibilities: [
        "Designed UI/UX in Photoshop and converted mockups to responsive HTML/CSS.",
        "Built frontend functionality with JavaScript, React, and jQuery.",
        "Integrated APIs using Axios with local caching and global error handling.",
        "Developed charts and data visualizations.",
        "Connected frontend to backend services.",
        "Implemented real estate calculation logic.",
      ],
    },
  ],
};

export const education = {
  icon: "/assets/resume/cap.svg",
  title: "My Education",
  description: "My academic journey and the foundations that shaped my skills.",
  items: [
    {
      institution: "University of Mindanao",
      degree: "Bachelor of Science in Information Technology (BSIT)",
      duration: "2013 - 2015",
    },
    {
      institution: "Computersense College of Communication & Technology",
      degree: "Computer Programming",
      duration: "2009 - 2011 ",
    },
    {
      institution: "PC Operation and Consumer Electronics",
      degree: "Ansuico Foundation Advance Technical School Inc.",
      duration: "2007 - 2008",
    },
  ],
};

export const skills = {
  title: "My Skills",
  description:
    "Technologies, tools, and skills I use to build modern web applications.",
  skillList: [
    {
      icon: <FaJs />,
      name: "javascript",
    },
    {
      icon: <FaReact />,
      name: "React.js",
    },
    {
      icon: <SiTypescript />,
      name: "TypeScript",
    },
    {
      icon: <SiNextdotjs />,
      name: "Next.js",
    },
    {
      icon: <SiSupabase />,
      name: "Supabase",
    },
    {
      icon: <SiTailwindcss />,
      name: "Tailwind CSS",
    },
    {
      icon: <FaNodeJs />,
      name: "node.js",
    },
    {
      icon: <SiExpress />,
      name: "Express.js",
    },
    {
      icon: <SiAuth0 />,
      name: "Auth0",
    },
    {
      icon: <SiJsonwebtokens />,
      name: "JWT",
    },
    {
      icon: <SiStripe />,
      name: "Stripe",
    },
    {
      icon: <SiRedux />,
      name: "Redux / Redux Toolkit",
    },
    {
      icon: <SiReactquery />,
      name: "TanStack React Query",
    },
    {
      icon: <SiShadcnui />,
      name: "shadcn/ui",
    },
    {
      icon: <SiVercel />,
      name: "Vercel",
    },
    {
      icon: <SiGit />,
      name: "Git",
    },
    {
      icon: <SiJest />,
      name: "Jest",
    },
    // {
    //   icon: <SiTestinglibrary />,
    //   name: "Testing Library",
    // },
    {
      icon: <SiPostgresql />,
      name: "PostgreSQL",
    },
    {
      icon: <SiMongodb />,
      name: "MongoDB",
    },
    {
      icon: <SiMysql />,
      name: "MySQL",
    },
    {
      icon: <SiSqlite />,
      name: "SQLite",
    },
    {
      icon: <SiGraphql />,
      name: "GraphQL",
    },
    {
      icon: <SiApollographql />,
      name: "Apollo",
    },
    // {
    //   icon: <SiVite />,
    //   name: "SiVite",
    // },
    {
      icon: <SiSocketdotio />,
      name: "Socket.IO",
    },
    {
      icon: <FaFigma />,
      name: "figma",
    },
    {
      icon: <SiPostman />,
      name: "Postman",
    },
    {
      icon: <SiAxios />,
      name: "Axios",
    },
    {
      icon: <SiEslint />,
      name: "ESLint",
    },
    {
      icon: <SiPrettier />,
      name: "Prettier",
    },
    {
      icon: <FaHtml5 />,
      name: "html 5",
    },
    {
      icon: <FaCss3 />,
      name: "CSS3",
    },
    {
      icon: <SiSass />,
      name: "Sass / SCSS",
    },
    // {
    //   icon: <SiStyledcomponents />,
    //   name: "styled-components",
    // },
    {
      icon: <SiBootstrap />,
      name: "Bootstrap",
    },
    {
      icon: <SiPhp />,
      name: "PHP",
    },
    {
      icon: <SiLaravel />,
      name: "Laravel",
    },
    // {
    //   icon: <SiXampp />,
    //   name: "XAMPP",
    // },
    // {
    //   icon: <SiSublimetext />,
    //   name: "Sublime Text",
    // },
    // {
    //   icon: <SiNotepadplusplus />,
    //   name: "Notepad++",
    // },
    // {
    //   icon: <SiAndroidstudio />,
    //   name: "Android Studio (Emulator)",
    // },
    {
      name: "NextAuth",
    },

    {
      name: "Zod",
    },
    {
      name: "tRPC",
    },
    {
      name: "Prisma",
    },
    {
      name: "Elasticsearch",
    },
    {
      name: "Redis",
    },
    {
      name: "Sentry",
    },
    {
      name: "MUI",
    },
    {
      name: "resend",
    },
    {
      name: "AWS S3",
    },
    {
      name: "AWS RDS",
    },

    {
      name: "Docker",
    },
    {
      name: "Jenkins (CI/CD)",
    },
    {
      name: "HTML5",
    },
    {
      name: "Playwright",
    },
    {
      name: "NoSQL",
    },
    {
      name: "Clerk",
    },
    {
      name: "jQuery",
    },
    {
      name: "WordPress",
    },
    {
      name: "React native",
    },
    {
      name: "Passport.js",
    },
    {
      name: "Oauth",
    },
    {
      name: "Nodemailer",
    },
    {
      name: "SignalR",
    },
    {
      name: "Maestro.io",
    },
    {
      name: "Ant Design",
    },
    {
      name: "VS Code",
    },
    {
      name: "pgAdmin",
    },
    {
      name: "DBeaver",
    },
    {
      name: "Redis Insight",
    },
    {
      name: "Kibana",
    },
    {
      name: "GIt Bash",
    },
    {
      name: "Docker Desktop",
    },
    {
      name: "MongoDB Compass",
    },
    {
      name: "Git Extension",
    },
    {
      name: "XAMPP",
    },
    {
      name: "WAMP",
    },
    {
      name: "LAMP",
    },
    {
      name: "AMPPS",
    },
    {
      name: "Workbench",
    },
    {
      name: "phpMyAdmin",
    },

    {
      name: "SQLyog",
    },
    {
      name: "Sublime Text",
    },
    {
      name: "Notepad ++",
    },
    {
      name: "Thunder Client",
    },
    {
      name: "Android Studio(emulator)",
    },
    {
      name: "Ollama",
    },
    {
      name: "DeepSeek R1",
    },
    {
      name: "Qwen 3",
    },
    {
      name: "Llama 3",
    },
    {
      name: "Cursor AI",
    },
    {
      name: "ChatGPT AI",
    },
    {
      name: "Claude AI",
    },
    {
      name: "Github Copilot",
    },
  ],
};

export const about = {
  title: "Basic Information",
  description: "General personal details for reference.",
  info: [
    {
      fieldName: "Name",
      fieldValue: "Marlon C. Hario",
    },
    // {
    //   fieldName: "Phone",
    //   fieldValue: "0968 740 5360",
    // },
    {
      fieldName: "Experience",
      fieldValue: "6+ years",
    },
    // {
    //   fieldName: "Skype",
    //   fieldValue: "oirah83",
    // },
    {
      fieldName: "Nationality",
      fieldValue: "Filipino",
    },
    // {
    //   fieldName: "Email",
    //   fieldValue: "hariomarlon83@gmail.com",
    // },
    {
      fieldName: "Civil Status",
      fieldValue: "Married",
    },
    {
      fieldName: "Gender",
      fieldValue: "Male",
    },
    {
      fieldName: "Age",
      fieldValue: "34",
    },
    // {
    //   fieldName: "Freelance",
    //   fieldValue: "Available",
    // },
    {
      fieldName: "Languages",
      fieldValue: "English, Tagalog, Visaya",
    },
    {
      fieldName: "Religion",
      fieldValue: "Roman Catholic",
    },
    {
      fieldName: "Place of Birth",
      fieldValue: "Badian, Cebu City PH",
    },
  ],
};

export const services = [
  {
    num: "01",
    title: "Senior React & Next.js Development",
    description:
      "Build scalable, responsive, and maintainable web applications using React, Next.js, and TypeScript.",
    href: "",
  },
  {
    num: "02",
    title: "Full-Stack Application Development",
    description:
      "Develop end-to-end SaaS and enterprise applications with modern frontend and backend technologies.",
    href: "",
  },
  {
    num: "03",
    title: "API Development & Integration",
    description:
      "Design and integrate REST APIs, GraphQL, authentication, and third-party services.",
    href: "",
  },
  {
    num: "04",
    title: "Performance & Frontend Optimization",
    description:
      "Improve application performance, code quality, accessibility, and user experience.",
    href: "",
  },
  {
    num: "05",
    title: "Testing & Quality Assurance",
    description:
      "Implement unit and end-to-end testing using Jest and Playwright to ensure reliable releases.",
    href: "",
  },
  {
    num: "06",
    title: "Legacy Modernization & Feature Development",
    description:
      "Enhance existing applications by refactoring code, fixing production issues, and delivering new features.",
    href: "",
  },
];

export const projects = [
  {
    num: "01",
    category: "",
    title: "Infinite Scroll Photo Gallery Application",
    description:
      "A simple infinite scroll image gallery that lets you explore photos seamlessly without pagination, just scroll and discover.",
    stack: [
      { icon: "", name: "Next.js" },
      { icon: "", name: "React.js" },
      { icon: "", name: "Tailwind v4" },
      { icon: "", name: "Tailwind CSS" },
      { icon: "", name: "shadcn/ui" },
    ],
    image: "/assets/work/lumina-cover.png",
    video: "",
    live: "https://infinite-scroll-gallery-lake.vercel.app/",
    github: "",
  },
  {
    num: "01",
    category: "",
    title: "Guest Checkout E-Commerce System",
    description:
      "A modern e-commerce system that allows users to browse products, add items to cart, and complete payments via Stripe without requiring user authentication, reducing friction and improving conversion rates.",
    stack: [
      { icon: "", name: "Next.js" },
      { icon: "", name: "React.js" },
      { icon: "", name: "TypeScript" },
      { icon: "", name: "Tailwind CSS" },
      { icon: "", name: "Radix UI" },
      { icon: "", name: "Swiper.js" },
      { icon: "", name: "Stripe" },
      { icon: "", name: "Vercel" },
    ],
    image: "/assets/work/ecommerce.png",
    video: "/assets/video/shop-ease.mp4",
    live: "https://shop-ease-kohl-two.vercel.app/",
    github: "https://github.com/marlonhario/ShopEase",
  },
  {
    num: "02",
    category: "",
    title: "Real-time Social & Matchmaking App",
    description:
      "TrueMatch is a real-time social app that allows users to connect and interact via chat and live streaming. Built with accessibility and performance in mind.",
    stack: [
      { icon: "", name: "Next.js" },
      { icon: "", name: "React.js" },
      { icon: "", name: "TypeScript" },
      { icon: "", name: "Tailwind CSS" },
      { icon: "", name: "Stream.io" },
      { icon: "", name: "Radix UI / shadcn" },
      { icon: "", name: "Supabase" },
      { icon: "", name: "Vercel" },
    ],
    image: "/assets/work/true-match-image.png",
    video: "/assets/video/true-match.mp4",
    live: "https://true-match-three.vercel.app/",
    github: "https://github.com/marlonhario/TrueMatch",
  },
  {
    num: "03",
    category: "",
    title: "Project Management Tool",
    description:
      "Board Mate is a Trello-inspired Kanban web application that enables users to visually manage tasks and workflows through draggable task boards. It includes secure authentication, dynamic board management, and a responsive dashboard interface, built using modern full-stack web technologies.",
    stack: [
      { icon: "", name: "Next.js" },
      { icon: "", name: "React.js" },
      { icon: "", name: "TypeScript" },
      { icon: "", name: "Tailwind CSS" },
      { icon: "", name: "Clerk" },
      { icon: "", name: "Radix UI / shadcn" },
      { icon: "", name: "DND Kit" },
      { icon: "", name: "Supabase" },
      { icon: "", name: "Vercel" },
    ],
    image: "/assets/work/boardmate-image.png",
    video: "/assets/video/boardmate.mp4",
    live: "https://board-mate-dun.vercel.app/",
    github: "https://github.com/marlonhario/BoardMate",
  },
];
